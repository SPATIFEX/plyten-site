...
file**, at `https://plyten.com/README.md` and `https://plyten.com/DEPLOYMENT.md`.
...
`CNAME` in the repository root contains `plyten.com`. GitHub Pages reads that file
and treats it as the custom domain, which means:

> The moment Pages is enabled, the `github.io` preview URL **redirects to
> plyten.com**. If DNS does not point at GitHub Pages yet, the preview does not
...
- **Preview before DNS.** Temporarily delete `CNAME`, enable Pages, check
  `https://spatifex.github.io/plyten-site/`, then restore `CNAME` before the domain
...
**Do not forget `www`.** `CNAME` names the apex only. Whatever `www.plyten.com`
...
- `canonical` on all three pages, and the three `hreflang` links on the two
  content pages, all pointing at plyten.com rather than at a preview URL;
