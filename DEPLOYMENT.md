...
`CNAME` in the repository root contains `plyten.com`. GitHub Pages reads that
...
- **Preview before DNS.** Temporarily delete `CNAME`, enable Pages, check
  `https://spatifex.github.io/plyten-site/`, then restore `CNAME` before the
...
- `canonical` and the three `hreflang` links point at plyten.com, not at a
  preview URL;
